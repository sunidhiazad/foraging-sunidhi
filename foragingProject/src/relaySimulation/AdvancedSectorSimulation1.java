...
//Communicate only to Hierarchical neighbors
public class AdvancedSectorSimulation1 {
	...
	public AdvancedSectorSimulation1(Cell food) {
		this.gridSize = 128;
		this.numAgents = 8;
		this.food = food;
	...
	private void checkPhaseCompletion(int clock) {
		...
		for (int i = 0; i < numAgents; i++) {
			...
			if (holder.getCurrPhase().equals(Phase.COMMUNICATION) && neighboursAware(holder)) {
				holder.setCurrPhase(Phase.TRANSPORTATION);
				if (!holder.getCommPhaseFlag().getCompleted())
					holder.setCommPhaseFlag(true, clock);
			}
	...
	public static void main(String[] args) {
		new AdvancedSectorSimulation1(new Cell(25, 36));
	}
}
