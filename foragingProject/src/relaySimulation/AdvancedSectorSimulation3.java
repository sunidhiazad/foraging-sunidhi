...
import commHelpers.AdvancedSectorComm;
...
//FoodLocator communicates to Hierarchical neighbors and one Quadrant neighbor
public class AdvancedSectorSimulation3 {
	...
	public AdvancedSectorSimulation3(Cell food) {
		this.gridSize = 128;
		this.numAgents = 8;
		this.food = food;
	...
	private void checkPhaseCompletion(int clock) {
		...
		for (int i = 0; i < numAgents; i++) {
			...
			// if (holder.getCurrPhase().equals(Phase.COMMUNICATION) &&
			// neighboursAware(holder) &&
			// agents.get(holder.getTransmitTo()).getCommPhaseFlag().getCompleted())
			// {
			// holder.setCurrPhase(Phase.TRANSPORTATION);
			// if (!holder.getCommPhaseFlag().getCompleted())
			// holder.setCommPhaseFlag(true, clock);
			...
			for (int i = 0; i < numAgents; i++) {
				...
				if (holder.getMove().equals(food)) {
					...
					if (!holder.getExPhaseFlag().getCompleted())
						holder.setExPhaseFlag(true, clock);
					holder.setCurrPhase(Phase.COMMUNICATION);
					holder.setReceivedFrom(holder.getId(), Pattern.ADVANCED_SECTORS);
					agents.set(i, holder);
					break;
				}
			}
			...
			for (int i = 0; i < aware.size(); i++) {
				...
				if (passTo.size() > 0) {
					for (int j = 0; j < passTo.size(); j++) {
						...
						ag.setReceivedFrom(receivedFrom, Pattern.ADVANCED_SECTORS);
						agents.set(id, ag);
					}
				}
			}
	...
	private Agent getNextCOMMmove(Agent ag, int clock) {
		...
		if (!neighboursAware(ag)) {
			if (ag.getPath().getPath().size() != 0)
				move = ag.getPath().getPath().remove(0);
			else 
				move = ag.getPath().getRepeat().getNext(ag.getMove());
		}else{
			if(ag.getFoodLocator()){
				if (ag.getCommPath() == null)
					ag = new AdvancedSectorComm(gridSize, numAgents, ag, food, clock, paths).getAgentWithCommPathSet();
	...
	private boolean neighboursAware(Agent ag) {
		for (int i = 0; i < ag.getHierTransmitTo().size(); i++) {
			if (!agents.get(ag.getHierTransmitTo().get(i)).getFoodLocAware())
				return false;
		}
		return true;
	}
	...
	public static void main(String[] args) {
		new AdvancedSectorSimulation3(new Cell(28, 8));
	}
}
